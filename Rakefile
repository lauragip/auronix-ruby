...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/fruizg0302/auronix-ruby/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("auronix-ruby.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in auronix-ruby.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "auronix-ruby.gemspec", /ruby_version = .*">= (.*)"/ => lowest
